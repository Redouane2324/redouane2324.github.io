...
app.post("/api/payments", async (req, res) => {
  try {
    ...
    res.json(formatPayment(result.rows[0]));
  } catch (error) {
    ...
    try {
      ...
      if (payment.status === "Paid") {
        return res.json({
          ...
          amount: Number(payment.verified_amount),
      ...
      const usedSignature = await pool.query(
        `
        SELECT payment_id
        FROM payments
        WHERE signature = $1
      ...
      if (usedSignature.rows.length > 0) {
        return res.json({
          paid: false,
          status: "SignatureAlreadyUsed",
      ...
      res.json({
        ...
        amount: Number(paid.verified_amount),
